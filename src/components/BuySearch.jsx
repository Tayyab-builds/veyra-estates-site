...
/**
 * Buy-only variant of PropertyFilters — reuses the same filter fields,
 * options data, and matchesFilters/countActiveFilters logic from the
 * Properties page (see RentalSearch, its rental counterpart), but drops
 * the Buy/Rent toggle since this page is dedicated to sale listings
 * (filters.purpose is always 'Buy').
 */
function BuySearch({ filters, onChange, onClear, resultCount, activeCount }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-2.5 border-b border-stone px-6 py-3.5 lg:px-7">
          <span className="h-1.5 w-1.5 rounded-full bg-olive" aria-hidden="true" />
          <p className="text-[0.7rem] uppercase tracking-wide2 text-plum/50">
            Searching Homes For Sale Only
          </p>
        </div>
          ...
          <div className="flex items-stretch">
            <div className="flex-1 grid grid-cols-5 divide-x divide-stone border-r border-stone">
              ...
              <button
                ...
                aria-controls="buy-more-filters-panel"
            ...
            {moreOpen && (
              ...
              >
                <div className="px-6 py-5">
                  <MoreFields idPrefix="buy-desktop" />
                </div>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div className="lg:hidden p-4 sm:p-5">
          ...
          >
            <span className="flex items-center gap-2.5 text-plum/70 text-sm">
              <FiSearch size={16} />
              {filters.search ? filters.search : 'Search Homes For Sale'}
            </span>
        ...
        {mobileOpen && (
          ...
          >
            ...
            <motion.div
              role="dialog"
              aria-modal="true"
              aria-label="Filter homes for sale"
              initial={{ y: '100%' }}
              animate={{ y: 0 }}
              exit={{ y: '100%' }}
            ...
            >
              <div className="flex items-center justify-between border-b border-stone pb-4">
                <h2 className="font-display text-xl text-plum">Filter Homes For Sale</h2>
                <button
                  type="button"
                  onClick={() => setMobileOpen(false)}
                  aria-label="Close filters"
              ...
              <div className="flex flex-col gap-5 pt-5">
                ...
                <details className="group border border-stone px-4 py-3 open:pb-4">
                  ...
                  <div className="pt-4">
                    <MoreFields idPrefix="buy-mobile" />
                  </div>
                </details>
              </div>
